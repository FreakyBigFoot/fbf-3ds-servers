...
func (pep *PRUDPEndPoint) handleConnect(packet PRUDPPacketInterface) {
	...
	ack.SetDestinationVirtualPortStreamType(packet.SourceVirtualPortStreamType())
	ack.SetDestinationVirtualPortStreamID(packet.SourceVirtualPortStreamID())
	// FFE experiment: nex-go zeroes this field. The console may require the
	// actual connection signature instead.
	switch ffeConnSigField() {
	case "ours":
		ack.SetConnectionSignature(connection.Signature)
	case "theirs":
		ack.SetConnectionSignature(packet.GetConnectionSignature())
	default:
	...
	if os.Getenv("FFE_SHOTGUN") == "1" {
		// Send every plausible CONNECT-ACK interpretation. The console keeps
		// whichever validates and ignores the rest; one retry tests them all.
		if v1, ok := ack.(*PRUDPPacketV1); ok {
			ffeShotgun(pep, connection, v1, packet)
			return
		}
	}
